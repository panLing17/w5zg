export default{
  install(Vue)
  {
    Vue.prototype.$initScroll = function (obj) {
      myScroll(obj)
    }
  }
}
...
function myScroll(ctx) {
  ...
  ctx.addEventListener("touchstart", function (e) {
    ...
    vy = 0;
    this._oy = e.touches[0].clientY - cur;//计算鼠标按下位置与列表当前位置的差值,列表位置初始值为0
    this._cy = e.touches[0].clientY;//鼠标按下的位置
  ...
  ctx.addEventListener("touchmove", function (e) {
    ...
    if (isDown) {//如果鼠标是从容器里开始滑动的
      if (e.timeStamp - this._startTime > 40) {//如果是慢速滑动，就不会产生力度，列表是跟着鼠标移动的
        ...
        cur = e.touches[0].clientY - this._oy;//列表位置应为 鼠标当前位置减去鼠标按下时与列表位置的差值,如:列表初始位置为0,鼠标在 5的位置按,那么差值为 5,此处假如鼠标从5滑动到了4,向上滑,cur = 4-5 =-1  ,假如鼠标从5滑动到了6,向下滑,cur= 6 - 5 = 1
...
//                        console.log(cur);
      ...
      vy = e.touches[0].clientY - this._cy;//记录本次移动后,与前一次鼠标位置的滑动的距离,快速滑动时才有效,慢速滑动时差值为 1 或 0,vy可以理解为滑动的力度

//                console.log(vy);
      this._cy = e.touches[0].clientY;//更新前一次位置为现在的位置,以备下一次比较
    }
